...
func generateEnumCode(pkgName string, tn *types.TypeName, cs []constNameAndString, kind constant.Kind, receiver string) (f *jen.File, err error) {
	defer func() {
		if r := recover(); r != nil {
			f = nil
			err = r.(error)
